...
pub(crate) fn target() -> Target {
    ...
    options.features = "+bulk-memory-opt,+sign-ext,+nontrapping-fptoint".into();
    options.endian = Endian::Big;

    options.os = "wasi".into();
    options.env = "p1".into();
    options.add_pre_link_args(LinkerFlavor::WasmLld(Cc::Yes), &["--target=wasm32be-wasip1"]);
    ...
    Target {
        llvm_target: "wasm32be-wasip1".into(),
        metadata: TargetMetadata {
            description: Some("WebAssembly (Big Endian) with WASI for W2C2".into()),
            tier: Some(2),
